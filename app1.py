...
st.write("""
         # Pneumothorax Detection via Deep Learning
         """
         )

st.write(" Enhanced Pneumothorax Diagnosis Through Deep Learning Driven Image Segmentation")
...
if file is None:
    ...
    if np.argmax(prediction) == 0:
        st.write("Prediction: Pneumothorax")
    elif np.argmax(prediction) == 1:
        st.write("Prediction: Normal")
    else:
        st.write("Prediction: Viral Pneumonia")
    
    st.text("Probability (0: Pneumothorax, 1: Normal, 2: Viral Pneumonia")
    st.write(prediction)
